...
async function main() {
  const summaryPath = path.join(visualDir, 'summary.json');
  const summary = JSON.parse(await fs.readFile(summaryPath, 'utf8'));
  await fs.mkdir(artifactDir, { recursive: true });
  await fs.mkdir(path.dirname(verdictStatePath), { recursive: true });

  const verdicts = [];
  for (const slide of summary) {
    const comparisonPath = path.resolve(visualDir, slide.comparison);
    const prompt = [
    ...
    const artifactPath = path.join(artifactDir, `claude-visual-review-slide-${slide.slide}-${timestamp}.md`);
    await fs.writeFile(
      artifactPath,
      [
        '# Original user task',
  ...
  const aggregate = {
    ...
    overall: {
      averageScore: verdicts.reduce((sum, item) => sum + Number(item.verdict.score || 0), 0) / Math.max(verdicts.length, 1),
      failingSlides: verdicts.filter((item) => item.verdict.verdict !== 'pass').map((item) => item.slide)
    },
    slides: verdicts
  };
